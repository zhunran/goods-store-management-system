  ...
  <div class="page-container">
    <el-card shadow="never">
      <div class="filter-bar">
        <el-input v-model="query.orderNo" placeholder="订单编号" clearable style="width: 220px" @keyup.enter="load" />
        <el-input v-model="query.memberAccount" placeholder="会员账号" clearable style="width: 180px" @keyup.enter="load" />
      ...
      <el-table :data="list" v-loading="loading" stripe>
        ...
        <el-table-column label="操作" width="180" fixed="right">
          <template #default="{ row }">
            ...
            <el-button v-if="row.status === '20'" size="small" type="primary" @click="ship(row)" v-permission="'order:ship'">
              发货
            </el-button>
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="detailVisible" title="订单详情" width="640px">
      <template v-if="detail">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="会员">{{ detail.memberAccount }}</el-descriptions-item>
          <el-descriptions-item label="状态">{{ statusText(detail.status) }}</el-descriptions-item>
          <el-descriptions-item label="金额">¥{{ detail.totalPay }}</el-descriptions-item>
          ...
          <el-descriptions-item label="收货地址" :span="2">{{ detail.receiverAddrDetail }}</el-descriptions-item>
          <el-descriptions-item label="备注" :span="2">{{ detail.orderComment || '-' }}</el-descriptions-item>
        </el-descriptions>
        <el-table :data="detail.items || []" style="margin-top: 16px" size="small" border>
...
async function load() {
  ...
  try {
    ...
    const data = await get<{ total: number; records: OrderVO[] }>('/order/admin/page', params)
...
async function ship(row: OrderVO) {
  await ElMessageBox.confirm(`确定对订单 ${row.orderNo} 发货吗？`, '提示', { type: 'warning' })
